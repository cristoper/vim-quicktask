...
"    }
function! QTParseTask(line, ...)
    let save_cursor = getcurpos()
    call cursor(a:line, 0)
    let start_line = s:FindTaskStart(1)
    let end_line = s:FindTaskEnd(0)
    let indent = s:GetTaskIndent()
    ...
    while current_line <= end_line
        ...
        let cur_indent = s:GetAnyIndent(current_line)
        ...
        if cur_indent < indent
            ...
            call s:EchoWarning("Reached unexpected end of task at line ".current_line)
            return task
        endif

        if line =~ s:task_or_section_regex && cur_indent > indent
            " this line is the start of a child task
            let subtask = QTParseTask(current_line, sections, depth)
        ...
        call s:EchoWarning("Skipping unknown line type: ".current_line)
        let current_line = current_line + 1
    endwhile
...
nmap <silent> <Plug>ShowWatchedTasksOnly     :call <SID>ShowWatchedTasksOnly()<CR>
nmap <silent> <Plug>ShowTodayTasksOnly       :call <SID>ShowTodayTasksOnly()<CR>
nmap <silent> <Plug>AddNextTimeToTask        :call <SID>AddNextTimeToTask()<CR>
